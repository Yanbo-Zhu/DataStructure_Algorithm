...
“快希选堆” 这几个都是 不稳定的排序 . 只要算法运行得快, 他就多半就不稳定  
...
第一轮 , 要做五次比较, 最大值就排到了最后 , 第二轮只要做4次比较就好了

• vergleiche sequentiell zwei Nachbarelemente und vertausche, wenn nötig
• mach das solange, bis keine Vertauschungen mehr stattfinden
• alle Elemente sind wie Luftblasen an ihren Platz „geblubbert“
...
![[04_Algorithm/image/Pasted image 20250125202741.png]]
...
![[04_Algorithm/image/Pasted image 20250125202938.png]]

![[04_Algorithm/image/Pasted image 20250125203016.png]]

### 3.3.1 Laufzeitkomplexitat 
...
位置k上的数和之前的数依次进行比较，找出这个数 在之前的数字中那个两个数之间 . 如果位置K上的数更大，将之前的数向后移位，最后将位置k上的数插入不满足条件点，反之不处理。

每从左向右, 在arrray提取一个值. 将这个值 和 左边的所有值进行 比较.   只向左看, 
向左 一位一位看, 比自己大, 就往左移动一位 . 再向左看, 又比自己大, 就再向左移动一位 , 就是位置交换 .  直到左边的值都比较完了
然后再看取 右边 大一index 位的 variable 的值,  和他的左边的值 全部进行比较, 进行位置交换

- an Sitzreihe entlanglaufen bis man einen kleineren Sitzenden gefunden hat
- alle bisherigen „größeren“ müssen eins nach rechts rutschen
	- das geht, weil ja ein Platz frei ist (der vom Aufgestandenen)
- alle Sitzplätze kommen der Reihe nach dran

![[04_Algorithm/image/Pasted image 20250125203755.png]]
...
Stabil?
- ja, gleichgroße Elemente tauschen nicht die Plätze
- 就是 sortiert 之后, 原来靠前面数在前面的数 和他同等值的数, 是否在 sortiert 后 依然靠前. 
- 

Verbesserung?
- Einfügestelle binär suchen
- dadurch nur ld j statt j-1 Vergleiche
- Verschiebeoperationen bleiben gleich
- evtl. durch effiziente Blockkopien verbesserbar

## 5.2 Laufzeitkomplexität
...
## 6.1 Eigenschaften, 复杂度

![[04_Algorithm/image/Pasted image 20250126181116.png]]
...
Hoares Scheme etwas effizienter: im Durchschnitt dreimal weniger Vertauschungen
Lomuto Scheme verschlechtert sich zu O(n2), wenn alle Elemente gleich sind

- zwei Indizes laufen aufeinander zu bis zur Mitte (Pivot), 之前都是用最右边的元素的值 作为 pivot 
- der linke Index sucht zu große Elemente zum Tauschen
- der rechte Index sucht zu kleine Elemente zum Tauschen

![[04_Algorithm/image/Pasted image 20250126181403.png]]


![[04_Algorithm/image/Pasted image 20250126181625.png]]
...
- Stabil?
	- ja, wenn der Merge-Schritt die Reihenfolge beachtet
- In-Place?
	- nein, lineare Speicherkomplexität (gemischte Arrays müssen zurückkopiert werden)
	- es ist aber möglich, das Mergen in-place zu implementieren, geht dann wieder auf Kosten der Laufzeit
- Verbesserung?
	- Parallelisieren
...
- die Gruppen werden von rechts nach links aufgefüllt
- der jeweilige Endindex wird nach links verschoben


`A[i]` = Element (gleichzeitig Schlüssel)
`C[A[i]]` = (restliche) Anzahl dieses Schlüssels
`B[C[A[i]]]` = Schlüssel an aktueller Gruppenposition einsortieren

![[04_Algorithm/image/Pasted image 20250126171933.png]]


## 12.4 Einsetzbarkeit

- Algorithmus nicht vergleichs-, sondern adressbasiert: gut, wenn Intervall {1, …, k} <<  Anzahl zu sortierender Elemente ist
- Beispiel (gut): Anzahl Einwohner bzgl. Alter
	- Einwohner: evtl. Millionen
	- Altersintervall: 0-130
- Beispiel (schlecht): Kurs nach Nachnamen sortieren
	- Gruppenstärke: 0-50
	- Namensintervall: allein deutsche Nachnamen ca. 850000
- Beispiel (Schwelle): koreanische Klassenstufe nach Nachnamen
	- koreanische Besonderheit: nur ca. 300 Familiennamen
...
In-Place?
- nein, benötigt Hilfarray (zum Zählen) abhängig von k

Stabil?
- ja, zählen und nacheinander einsortieren ändert Reihenfolge nicht, wenn die Eingabe von hinten durchlaufen wird (Zeile 8)
...
vermeidet Vergleiche durch Einsortieren nach Stellenwert in „Basisstellen-Container“

![[04_Algorithm/image/Pasted image 20250125211827.png]]
...
算法就是 如果 这个 array 不是 sorted 好的,  就将 这个 array 随机 无章法的洗乱  (randomly permute )

B(N):若  array 是 sorted好的,  只要 比较n 次 , 就结束了

![[04_Algorithm/image/Pasted image 20250125212622.png]]
